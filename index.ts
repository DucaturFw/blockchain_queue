...
const ethListen = async (cb: (a: EventLog) => void, err: (a: Error) => void) => {
  try {
    ...
    ctr.events.allEvents({ fromBlock: 0 }, (error, event) => error ? err(error) : cb(event))
  } catch (error) {
    err(error)
  }
}
...
const main = async () => {
  try {
    ...
    await ethListen(insertIntoDb(conn), console.log)
  } catch (err) {
    console.log({ err })
  }
}
