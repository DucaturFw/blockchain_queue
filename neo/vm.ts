...
import { IApplogTx } from './types'

const { parse } = require('neo-script-parser')
...
export let juxt = (...funcs: Array<(a: string) => any>) => (...args: any[]) => args.map((x, idx) => funcs[idx](x))
export let CONV = {
...
export function parseAnyCall(method: string, ...args: Array<(a: string) => any>) {
  return (script: string) => {
    const call = parseContractCall(script)
